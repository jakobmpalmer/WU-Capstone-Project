...
class CarActivity : AppCompatActivity() {
    ...
    private var fuel : Double = 21.25
    private var mileage : Double = 19.73
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        db.collection(userPath).document("mileage_selected").get().addOnSuccessListener {result ->
            var mileage = result?.get("mpg").toString().toFloatOrNull()!!.toDouble()
            println("FOUND DOUBLEE: $mileage")
        }.addOnFailureListener{
            println("Failed to read mileage_selected")
        }
        ...
        db.collection(userPath).document("fuel_selected").get().addOnSuccessListener {result ->
            ...
            println("FOUND DOUBLEEE: $fuel \n")
        }.addOnFailureListener{
            println("Failed to read fuelRate")
        }
...
//
          // var mileage = this.db.collection(userPath).document("mileage_selected").get().result.toString().toDouble()
//
//            var thisCarbonFp = calcImpact(mileage, fuel)
//
...
//                var mileage = result?.get("mpg").toString().toFloatOrNull()!!.toDouble()
//                }.addOnFailureListener{
//                    println("Failed to read mileage") // Falied to get mileage
//                var mileage = 19.73
//                var thisCarbonFp = calcImpact(mileage, fuel)
//                    storeData(getOurDate(), getOurTime(), thisCarbonFp)
//                }
//            }.addOnFailureListener{
//                println("Failed to read mileage_selected")
//                db.collection(userPath).document("fuel_selected").get().addOnSuccessListener {result ->
//                    fuel = result?.get("fuel_rate").toString().toFloatOrNull()!!.toDouble() // Mileage fails, fuel Succeed
//                    println("FOUND DOUBLEEE: $fuel \n")
//                    var thisCarbonFp = calcImpact(mileage, fuel)
//
//                    storeData(getOurDate(), getOurTime())
//                }.addOnFailureListener{
//                    println("Failed to read fuelRate") // Falied to get Fuel
//                    var thisCarbonFp = calcImpact(mileage, fuel)
//
//                    storeData(getOurDate(), getOurTime())
//                }
...
//        println("Our actual fuel: $fuel")
        ...
        //val tempCarbonFootprint = calcImpact(fuel_rate = fuel, miles_per_gal = mileage)
        //----
    ...
    val carEmission = ((carData-walkData)/mileage) * fuel
    println("EQ=:: ( $carData - $walkData / $mileage ) * $fuel")
    ...
    //----
        ...
        val tempCarbonFootprint = (carEmission + busEmission + planeEmission)
        ...
        val data = hashMapOf(
            ...
            "carbon_footprint" to tempCarbonFootprint
        )
        ...
        travelTotalRef.get().addOnSuccessListener { result ->
            ...
            print("Here Here! " + tempCarbonFootprint + " + " + if (oldCarbonFpTotal != null) oldCarbonFpTotal else 0f)
            var newCarbonFpTotal = tempCarbonFootprint + if (oldCarbonFpTotal != null) oldCarbonFpTotal else 0f
...
//            }
    ...
    fun calcImpact(fuel_rate : Double, miles_per_gal : Double): Double{
